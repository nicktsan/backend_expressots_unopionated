...
import { IDeckslotUpdateQuantityRequestDto, IDeckslotUpdateQuantityResponseDto } from "./deckslot-update-quantity.dto";
...
export class DeckslotUpdateQuantityUsecase {
    ...
    public async execute(payload: IDeckslotUpdateQuantityRequestDto, userId: string): Promise<IDeckslotUpdateQuantityResponseDto | AppError> {
        try {
            ...
            if (res.quantity < 1) {
                ...
                const deleteRes: IDeckslotDeleteResponseDto | null = await this.deckSlotRepository.deleteOneDeckSlot(deletePayload);
                if (!deleteRes) {
                    const error = this.report.error(
