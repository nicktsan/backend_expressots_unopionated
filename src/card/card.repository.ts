...
export class CardRepository extends BaseRepository<CardEntity>{
    ...
    private buildCardQuery(payload: ICardFindRequestDto): SQL<unknown> {
        ...
        if (payload.code && typeof payload.code === 'string') {
            conditions.push(like(cards.code, `%${this.sanitizeInput(payload.code)}%`));
        }
        
        if (Array.isArray(payload.rarity) && payload.rarity.length > 0) {
            conditions.push(inArray(cards.rarity, payload.rarity.map(this.sanitizeInput)));
        }
        
        if (Array.isArray(payload.card_type) && payload.card_type.length > 0) {
            conditions.push(inArray(cards.card_type, payload.card_type.map(this.sanitizeInput)));
        }
        
        if (Array.isArray(payload.color) && payload.color.length > 0) {
            conditions.push(inArray(cards.color, payload.color.map(this.sanitizeInput)));
        }
        
        if (Array.isArray(payload.card_level) && payload.card_level.length > 0) {
            conditions.push(inArray(cards.card_level, payload.card_level.map(Number)));
        }
        
        if (payload.plain_text_eng && typeof payload.plain_text_eng === 'string') {
            conditions.push(like(cards.plain_text_eng, `%${this.sanitizeInput(payload.plain_text_eng)}%`));
        }
        
        if (payload.plain_text && typeof payload.plain_text === 'string') {
            conditions.push(like(cards.plain_text, `%${this.sanitizeInput(payload.plain_text)}%`));
        }
        
        if (Array.isArray(payload.expansion) && payload.expansion.length > 0) {
            conditions.push(inArray(cards.expansion, payload.expansion.map(this.sanitizeInput)));
        }
        
        if (payload.illustrator && typeof payload.illustrator === 'string') {
            conditions.push(like(cards.illustrator, `%${this.sanitizeInput(payload.illustrator)}%`));
        }
